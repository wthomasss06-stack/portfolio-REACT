...
const Beams = ({
  ...
  float getPos(vec3 pos) {
    return cnoise(vec3(pos.x * 0., pos.y - uv.y, pos.z + time * uSpeed * 3.) * uScale);
  }
  vec3 getCurrentPos(vec3 pos) { vec3 np = pos; np.z += getPos(pos); return np; }
  vec3 getNormal(vec3 pos) {
    vec3 cp = getCurrentPos(pos);
    vec3 tx = normalize(getCurrentPos(pos + vec3(0.01,0.,0.)) - cp);
    vec3 tz = normalize(getCurrentPos(pos + vec3(0.,-0.01,0.)) - cp);
    ...
    gl_FragColor.rgb -= rn / 15. * uNoiseIntensity;`
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB('#FF5500')),
          time: { value: 0 },
          roughness: 0.1,
          metalness: 0.6,
          uSpeed: { value: speed },
...
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      if (j < heightSegments) {
        const a = vertexOffset, b = a + 1, c = a + 2, d = a + 3;
