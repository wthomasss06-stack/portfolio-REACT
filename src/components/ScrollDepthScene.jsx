...
function useDepthEntranceGSAP(containerRef) {
  useEffect(() => {
    ...
    const sections = [...container.querySelectorAll('.sdz-section')]
      .slice(1)
      .filter((s) => !s.classList.contains('sdz-section--no-depth'));
    ...
    <>
      ...
      <div ref={containerRef} style={{ position: 'relative', zIndex: 2 }}>
        {React.Children.map(children, (child, index) => {
          const skipDepth = !!(child && child.props && child.props.sdzSkip);
          return (
            <div
              key={index}
              className={`sdz-section${skipDepth ? ' sdz-section--no-depth' : ''}`}
              style={
                skipDepth
                  ? undefined
                  : { willChange: 'transform, opacity, filter', transformOrigin: 'center center' }
              }
            >
              {child}
            </div>
